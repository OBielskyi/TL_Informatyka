...
from random import (
    randint,
)  # lub import random, ale dla funkcjonowania tego programu potrzebny jest tylko sam randint
...
while m > 0:  # Cykl powtarza się dopóki nie stracimy wszystko
    ...
    match wybor:
        ...
        case _:
            ...
            exit()
    stawka = float(
        input("Wprowadź wysokość stawki [zł]: ")
    )  # Tak, możemy odjąć float od int. Python jest mądry.
    if stawka > m:
        print(
            "Niewystarczająca ilość pieniędzy."
        )  # Oczywiście, że stawka nie może być większą od m
    ...
    if (
        i != 0
    ):  # 0 nie może być ani parzyste, ani nieparzyste (Najman zabije mnie za to, wiem)
        if i % 2 == 0:
    ...
    if win:
        m = (
            m + q * stawka
        )  # Jeśli zapomniałeś co to było q, to czytaj komentarze na samym początku
        print("Wygrana!")
    else:
        print("Przegrana!")
